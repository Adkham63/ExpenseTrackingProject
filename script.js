...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelector(".closeDailyTransaction")
    .addEventListener("click", closeDayPopup);
});
...
function exportTransactions() {
  ...
  transactions.forEach((t) => {
    ...
    if (t.isFixedCost) {
      transactionDetails = `Fixed Cost - Name : ${
        t.name
      }, Amount: ${t.amount.toFixed(2)}`;
    } else {
      let formattedDate = `${t.date.getDate()}/${
        t.date.getMonth() + 1
      }/${t.date.getFullYear()}`;
      transactionDetails = `Daily transaction - Date: ${formattedDate}, Name: ${
        t.name
      }, Amount: ${t.amount.toFixed(2)}`;
    }
...
function updateDoneTransactionsDisplay() {
  ...
  transactions.forEach((t) => {
    ...
    if (t.isFixedCost) {
      ...
      formattedDate = `${t.date.getDate()}/${
        t.date.getMonth() + 1
      }/${t.date.getFullYear()}`;
    }

    transactionElement.textContent = `${formattedDate}: ${t.name} - ${
...
document
  .getElementById("openFixedCostsPopup")
  .addEventListener("click", openFixedCostsPopup);
document
  .querySelector(".closeFixedCosts")
  .addEventListener("click", closeFixedCostsPopup);
